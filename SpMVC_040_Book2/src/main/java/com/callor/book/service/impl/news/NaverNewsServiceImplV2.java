...
import com.callor.book.controller.NaverParentDTO;
...
/*
 * spring framework의 RestTemplate 클래스를 사용한 JSON parsing
 *json-simple, Gson등을 사용할때는 
 *
 *OpenAPI를 통하여 JsonString 을 수신하고
 *
 *parsing을 통해 DTO , List<DTO>형태의 데이터로 변환하는 과정을 수행했다.
 *
 *
 *RestTemplate을 사용하면
 *DTO(VO)의 겹치는 구조를 잘설계하고
 *요청주소를 URI로 생성하고
 *인증정보등을 HttpHeaders클래스로 설정하고 
 *HttpEntity로 변환하여 RestTempate에 주입한다.
 *
 *
 ...
 *
 *
 *내부에서 요청정보를 OpenAPI로 보내고 수신된 데이터를 DTO (VO)구조에 따라 자체 parsing을 하고 
 *
 *결과를 돌려준다
 *
 */
...
/* json - simple gGson 등으로 parsing을 수행할때는
 * 
 * URL 클래스를 사용하여 naver에 요청하고 결과를 jsonString으로 받았다
 * 
...
//		spring 순수의 클래스
//		Spring framework 3.2에서 처음 도입된 클래
//		openapi에 요청을 하기위한 클래
		RestTemplate restTemp = new RestTemplate();

		URI restURI = new URI(queryURL);
		
/*
 * naver에 요청할때 사용할 인증 정보를 httpprotcol의 header 에 부착하기
 */
		HttpHeaders header = new HttpHeaders();
		
		header.set("X-NAVER-Client-Id",NaverSecret.NAVER_CLIENT_ID);
		header.set("X-NAVER-Client-Id",NaverSecret.NAVER_CLIENT_SECRET);
		...
		log.debug("업데이트 날짜 {} ", resList.getBody().total);
		log.debug("업데이트 날짜 {} ", resList.getBody().items.toString());
